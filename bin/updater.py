...
def download_dbsnp_vcf(dbsnp_build=None,genome_build=None,url=None,outpath=None):
  ...
  TEMPLATE_URL = "ftp://ftp.ncbi.nlm.nih.gov/snp/organisms/human_9606_{0}_{1}/VCF/00-All.vcf.gz"
  ...
  if url is None:
    ...
    url = TEMPLATE_URL.format(dbsnp_build,genome_build)
  ...
  with tqdm(unit='B',unit_scale=True,miniters=1,desc=url.split('/')[-1]) as t:
    urllib.urlretrieve(url,filename=outpath,reporthook=tqdm_hook(t),data=None)
...
def create_db(vcf_path,db_path,rsmerge_path,snphistory_path,chunksize=500000):
...
def get_settings():
  ...
  p.add_option("--db",help="Database name or path to existing database.")
...
def main():
  ...
  if not os.path.isfile(db_name):
    ...
    print("Downloading NCBI dbSNP VCF for {} / {}".format(DBSNP_BUILD,GENOME_BUILD))
    vcf_path = download_dbsnp_vcf(DBSNP_BUILD,GENOME_BUILD)

    # Create sqlite database
    create_db(vcf_path,db_name,rsmerge_path,snphistory_path)
  ...
  if opts.gwas_cat:
    ...
    final_gwas_cat = "gwascat_ebi_%s.tab" % GENOME_BUILD
